#!/usr/bin/env python3
"""Benchmark for 11 working frameworks"""
import json
import time
import statistics
...
# 11 frameworks that are currently running
FRAMEWORKS = {
    'jackson': {'port': 8081, 'name': 'Jackson JSON', 'endpoint': '/api/jackson/benchmark/serialization'},
    'avro': {'port': 8083, 'name': 'Apache Avro', 'endpoint': '/api/avro/benchmark/serialization'},
    'kryo': {'port': 8084, 'name': 'Kryo', 'endpoint': '/api/kryo/benchmark/serialization'},
    'msgpack': {'port': 8086, 'name': 'MessagePack', 'endpoint': '/api/msgpack/benchmark/serialization'},
    'flatbuffers': {'port': 8091, 'name': 'FlatBuffers', 'endpoint': '/api/flatbuffers/benchmark/serialization'},
    'grpc': {'port': 8092, 'name': 'gRPC', 'endpoint': '/api/grpc/benchmark/serialization'},
    'cbor': {'port': 8093, 'name': 'CBOR', 'endpoint': '/api/cbor/benchmark/serialization'},
    'bson': {'port': 8094, 'name': 'BSON', 'endpoint': '/api/bson/benchmark/serialization'},
    'arrow': {'port': 8095, 'name': 'Apache Arrow', 'endpoint': '/api/arrow/benchmark/serialization'},
    'sbe': {'port': 8096, 'name': 'SBE', 'endpoint': '/api/sbe/benchmark/serialization'},
    'parquet': {'port': 8097, 'name': 'Apache Parquet', 'endpoint': '/api/parquet/benchmark/serialization'}
}
...
def test_framework(framework_key, framework_info, scenario):
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            return {'success': True, 'response_time_ms': response_time, 'data': data}
        else:
...
def main():
    ...
    for framework_key, framework_info in FRAMEWORKS.items():
        ...
        framework_results = {'name': framework_info['name'], 'port': framework_info['port'], 'scenarios': {}}
    ...
    # Rankings
    sorted_frameworks = sorted(results.items(), key=lambda x: x[1]['summary']['overall_success_rate'], reverse=True)

    print(f"\n🏆 FRAMEWORK RANKING:")
    print("-" * 60)
    print(f"{'Rank':<6} {'Framework':<25} {'Success':<10} {'Avg Time':<12} {'Status'}")
    print("-" * 60)
    ...
    for i, (framework_key, result) in enumerate(sorted_frameworks, 1):
        ...
        print(f"{i:4}. {result['name']:25} {success_rate:6.1f}%    {avg_time:8.2f}ms    {status}")
    ...
    print(f"\n🎉 BENCHMARK COMPLETE! Tested {len(results)} frameworks")
