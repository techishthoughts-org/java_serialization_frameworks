...
/**
 * SBE Serialization Service (V2 - using AbstractSerializationService)
 *
 * Provides SBE (Simple Binary Encoding) serialization using JSON fallback.
 * SBE is optimized for ultra-low latency in financial trading.
 *
 * Note: Uses JSON fallback for simplicity. Production should use SBE code generation.
 *
 * Features:
 * - Extends AbstractSerializationService for unified benchmarking
 * - JSON-based serialization (SBE fallback)
 * - GZIP compression support
 * - Memory monitoring integration
 * - Roundtrip testing
 */
@Service
public class SbeSerializationServiceV2 extends AbstractSerializationService {

    private final ObjectMapper objectMapper;

    public SbeSerializationServiceV2(UnifiedPayloadGenerator payloadGenerator) {
    ...
    public String getFrameworkName() {
        return "SBE (Simple Binary Encoding)";
    }

    @Override
    public String getTypicalUseCase() {
        return "Ultra-low latency applications and high-frequency trading";
    }

    @Override
    public boolean supportsSchemaEvolution() {
        return false; // SBE requires schema compatibility
    }
    ...
    public SerializationResult serialize(List<User> users) throws SerializationException {
        ...
        try {
            ...
            throw new SerializationException(
                    getFrameworkName(),
                    "SERIALIZE",
                    "Failed to serialize users with SBE",
                    e
            );
        }
    }
    ...
    public List<User> deserialize(byte[] data) throws SerializationException {
        try {
            ...
            throw new SerializationException(
                    getFrameworkName(),
                    "DESERIALIZE",
                    "Failed to deserialize users with SBE",
                    e
            );
        }
    }
